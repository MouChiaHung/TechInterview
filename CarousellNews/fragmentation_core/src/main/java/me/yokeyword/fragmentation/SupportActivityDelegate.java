...
public class SupportActivityDelegate {
    ...
    public void onBackPressed() {
        ...
        mTransactionDelegate.mActionQueue.enqueue(new Action(Action.ACTION_BACK) {
            @Override
            public void run() {
                if (isLog) Logger.d("************** run ************** ");
                if (!mFragmentClickable) {
                    mFragmentClickable = true;
                }

                // 获取activeFragment:即从栈顶开始 状态为show的那个Fragment
                if (isLog) Logger.d("... going to find active fragment");
                ISupportFragment activeFragment = SupportHelper.getActiveFragment(getSupportFragmentManager());
                if (isLog && activeFragment != null) Logger.d("... active fragment:" + activeFragment.getClass().getSimpleName());
                if (isLog && activeFragment == null) Logger.e("!!! activeFragment is null");
    ...
    public void onBackPressedSupport() {
        ...
        if (getSupportFragmentManager().getBackStackEntryCount() > 1) {
            if (isLog) Logger.d("... getSupportFragmentManager().getBackStackEntryCount() > 1" + getSupportFragmentManager().getBackStackEntryCount());
            if (isLog) Logger.d("... going to pop()");
            pop();
        } else {
            if (isLog) Logger.d("... getSupportFragmentManager().getBackStackEntryCount() <= 1:" + getSupportFragmentManager().getBackStackEntryCount());
            if (isLog) Logger.d("... going to ActivityCompat.finishAfterTransition()");
