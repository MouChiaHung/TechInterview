...
class TransactionDelegate {
    ...
    boolean dispatchBackPressedEvent(ISupportFragment activeFragment) {
        ...
        if (activeFragment != null) {
            if (isLog) Logger.d("... active fragment:" + activeFragment.getClass().getSimpleName());
            boolean result = activeFragment.onBackPressedSupport();
            if (result) {
                return true;
            }
            if (isLog) Logger.d("... going to call parent fragment:" + ((Fragment) activeFragment).getParentFragment().getClass().getSimpleName());
            ...
            if (isLog) Logger.e("!!! activeFragment is null");
        }
        return false;
    }
    ...
    void handleResultRecord(Fragment from) {
        try {
            Bundle args = from.getArguments(); //the bundle which to retrieve the fragment reference from
            if (args == null) return;
    ...
    private void doDispatchStartTransaction(FragmentManager fm, ISupportFragment from, ISupportFragment to, int requestCode, int launchMode, int type) {
        ...
        if (isLog) Logger.d("... going to start() and dontAddToBackStack:" + dontAddToBackStack);
    ...
    private void start(FragmentManager fm, final ISupportFragment from, ISupportFragment to, String toFragmentTag,
        ...
        if (isLog) Logger.d("... dontAddToBackStack:" + dontAddToBackStack);
    ...
    /**
     * saves request code and a reference to a fragment which comes previously
     */
    private void saveRequestCode(FragmentManager fm, Fragment from, Fragment to, int requestCode) {
        Bundle bundle = getArguments(to); //the bundle which to put the fragment reference in
    ...
    /**
     * on specified fragment manager, its temporary back stack could be...
     * a BackStackRecord without a assigned name means not being added to back stack
     * , try to create a new BackStackRecord which holds remove() commands on willPopFragments and commit it
     * or a BackStackRecord with a assigned name means being added to back stack
     * , try to enqueue the action which pops back stacks in order from the top back stack to the back stack which has the same name
     *
     * FragmentManagerImpl#popBackStackState() takes as many FragmentManagerImpl#mBackStack as possible into FragmentManagerImpl#mTmpRecords
     * until getting the targeted BackStackRecord which has the same assigned name as BackStackRecord#mName given by caller
     *
     * this method adopts both of two strategies
     */
